...
function Habit(name, difficulty, days, notes){
    ...
    var d = new Date();
    this.getTaskToday = function(){return this.days[d.getDay()];}
...
var CreateGoalModule = (function(){
    ...
    createGoalBtn.onclick = function(event){
        addNewGoal();
        //Hide modal
        document.getElementById("addGoalBox").style.display = "none";       
    }

    //Public function to add goal to the DOM
    return{
        appendGoal: function(newGoal){
                var goalBlock = "<div id=\"" + newGoal.getName() + "\" class=\"goalBlock\">"
                                + "<strong>" + newGoal.getName() + "</strong>" + "<br>"
                                + "<div class=\"goalInfoBlock\" id=\"" + newGoal.getName() + "GoalInfoBlock\">"
                                + "Category:        " + newGoal.getCategory()  + "<br>"
                                + "<div id=\"" + newGoal.getName() + "_currSco\">Current score:   " + newGoal.getCurrentScore() + "</div><br>"
                                + "<div id=\"" + newGoal.getName() + "_highSco\">High score:      " + newGoal.getHighScore() +"</div><br>"
    ...
    var editInfoModal = function(){
        ...
        for(i = 0; i < editInfoBtn.length; i++){
            ...
            editInfoBtn[i].onclick = function(event){
                ...
                for(j = 0; j < modalBoxes.length; j++){
                    modalBoxes[j].style.display = "none";
                }
                ...
                saveChangesBtn.onclick = function(){
                    ...
                    //Hide the modal box
                    document.getElementById("editGoalBox").style.display = "none";
...
var FilterGoals = (function(){
    //Relevant elements
    dropDownBtn = document.getElementById("filterGoalsBtn");
    btnList = document.getElementById("goalsFilterBtns");
    filterBtns = document.getElementsByClassName("categBtn");
    //Is the list open or closed?
    var openList = false;

    //What to do when the filter button is clicked
    dropDownBtn.onclick = function(){
        //If list is open, remove all filter buttons
        if(openList){
            while(btnList.hasChildNodes()){
                btnList.removeChild(btnList.lastChild);
            }
            openList = false;
        //If list is closed, append all filter buttons
        } else{
...
var appendHabbit = function(habit){
    ...
    var habitBlock = "<div class=\"habitBlock\" id=\"" + habit.getName() + "_hab\">"
                    + "<button id=\"" + habit.getName() + "_habFin\" class=\"habFinBtn\">Check</button>" 
                    + "<strong>" + habit.getName() + "</strong><br><br>"
                    + "<div class=\"habitInfoBlock\" id=\"" + habit.getName() + "_habInfo\">" 
                    + habitTodo()
                    + "Difficulty:  " + habitDifficulty() + "<br>"
                    ...
                    + "Notes:<br><i>" + habit.getNotes() + "</i><br><br>"
            ...
            return {
                ...
                hideHabbitBoxes: function(){
                    for(i = 0; i < habitInfoBx.length; i++){
                        habitInfoBx[i].style.display = "none";
                    }
...
var addNewHabit = (function(){
    ...
    var deleteHabit = (function(){
        ...
        document.getElementById("deleteHabitBtn").onclick = function(event){
            ...
            document.getElementById("editHabitBox").style.display = "none";            
        }
